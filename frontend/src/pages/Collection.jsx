...
const Collection = () => {
  ...
  const [title, setTitle] = useState('ALL COLLECTIONS'); // State to manage the title
  ...
  const updateTitle = () => {
    if (category.length > 0) {
      ...
      } else if (category.includes('Unisex')) {
        setTitle('Nước Hoa Unisex');
      }
    } else if (subCategory.length > 0) {
      setTitle(`Nước Hoa ${subCategory.join(', ')}`);
    } else {
      ...
      <div className='min-w-60'>
        ...
        <div className={`border border-gray-300 pl-5 py-3 my-5 ${showFilter ? '' : 'hidden'} sm:block`}>
          ...
          <div className='flex flex-col gap-2 text-sm font-light text-gray-700'>
            ...
            <p className='flex gap-2'>
              <input className='w-3' type="checkbox" value={'Calvin Klein'} onChange={toggleSubCategory} /> Calvin Klein
            </p>
            <p className='flex gap-2'>
      ...
      <div className='flex-1'>
        <div className='flex justify-between text-base sm:text-2xl mb-4'>
          ...
          <select onChange={(e) => setSortType(e.target.value)} className='border-2 border-gray-300 text-sm px-2'>
            <option value="relevant">Sắp xếp theo: Có liên quan</option>
